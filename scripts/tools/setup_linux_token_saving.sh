...
# ==============================================================================
# VoltFlow POS / Himmel — Linux Token-Saving Agent Setup Script
# Replicates full token-saving environment from Windows desktop to Linux:
# - tokless orchestrator (rtk, caveman, ponytail, codegraph, context-mode)
# - Antigravity global instructions (~/.gemini/GEMINI.md)
# - Antigravity PreToolUse & PreInvocation hooks (RTK, Loop Detector, Caveman)
# - MCP servers: codegraph, context-mode, serena, openrouter, ollama
# - Custom skills: phase-auto, phase-plan, cavecrew
# ==============================================================================
...
if [ ${#MISSING_DEPS[@]} -gt 0 ]; then
    ...
    echo "Please install them via your package manager, e.g.:"
    echo "  sudo apt-get update && sudo apt-get install -y curl git python3 python3-pip python3-venv nodejs npm ripgrep"
    exit 1
fi

echo "Prerequisites OK: curl, git, python3, node, npm found."

# ==============================================================================
# 2. Global Node Tools (codegraph, context-mode)
# ==============================================================================
echo "=== [2/6] Installing Global Node Tools ==="
npm install -g @colbymchenry/codegraph context-mode || {
    echo "Warning: Global npm install failed with current permissions. Trying with user prefix..."
...
# ==============================================================================
# 3. Install tokless CLI & Wire Core Tools
# ==============================================================================
echo "=== [3/6] Installing & Configuring tokless ==="
if ! command -v tokless &>/dev/null; then
    echo "Installing tokless via official install script..."
...
if command -v tokless &>/dev/null; then
    echo "Wiring tokless tools for agents (antigravity, claude, opencode)..."
    tokless --agents antigravity,claude,opencode || true
else
    echo "Warning: tokless binary not found in PATH (~/.local/bin)."
fi

# ==============================================================================
# 4. Create Directory Hierarchy
# ==============================================================================
echo "=== [4/6] Creating ~/.gemini Directory Hierarchy ==="
mkdir -p "$HOME/.gemini/antigravity"
mkdir -p "$HOME/.gemini/config/scripts"
mkdir -p "$HOME/.gemini/config/skills/phase-auto"
mkdir -p "$HOME/.gemini/config/skills/phase-plan"
mkdir -p "$HOME/.gemini/config/skills/cavecrew"

# ==============================================================================
# 5. Write Configuration Files & Scripts
# ==============================================================================
echo "=== [5/6] Writing Antigravity Global Settings & Hooks ==="
...
# --- ~/.gemini/config/scripts/loop_detector.py ---
cat << 'EOF' > "$HOME/.gemini/config/scripts/loop_detector.py"
#!/usr/bin/env python3
import hashlib
import json
import os
...
# --- ~/.gemini/config/scripts/caveman_interceptor.py ---
cat << 'EOF' > "$HOME/.gemini/config/scripts/caveman_interceptor.py"
#!/usr/bin/env python3
import json
import os
import re
...
chmod +x "$HOME/.gemini/config/scripts/loop_detector.py"
chmod +x "$HOME/.gemini/config/scripts/caveman_interceptor.py"

# --- ~/.gemini/config/hooks.json ---
cat << 'EOF' > "$HOME/.gemini/config/hooks.json"
{
  "rtk": {
    "PreToolUse": [
      {
...
cat << EOF > "$HOME/.gemini/config/mcp_config.json"
{
  "mcpServers": {
    ...
    "ollama": {
      "command": "python3",
      "args": [
        "$PROJECT_DIR/mcp_ollama_server.py"
      ],
      "env": {
    ...
    "openrouter": {
      "command": "python3",
      "args": [
        "$PROJECT_DIR/mcp_openrouter_server.py"
      ],
      "env": {
...
cat << EOF > "$HOME/.gemini/antigravity/mcp.json"
{
  "mcpServers": {
    "serena": {
      "command": "serena",
...
# --- Skills ---
cat << 'EOF' > "$HOME/.gemini/config/skills/phase-auto/SKILL.md"
---
name: phase-auto
description: >-
  Fully autonomous multi-phase planner and subagent orchestrator with branch isolation. Runs all phases in an isolated branch (Workspace: 'branch'), prompts user to test, squash-merges with Conventional Commit upon confirmation, and deletes temporary branch.
---

# Phase Auto (Autonomous Branch-Isolated Orchestrator)

Autonomously partition complex tasks, execute all phases in an isolated branch (\`Workspace: 'branch'\`), prompt user for manual testing, squash-merge with Conventional Commit, and auto-delete scratch branches.

## Workflow

### 1. Auto-Partition
- Analyze request and partition into sequential phases (\`Phase 1\`, \`Phase 2\`, ...).
- Output brief roadmap and immediately launch execution without blocking.

### 2. Autonomous Isolated Execution
For each phase in sequence:
1. **Launch Phase in Isolated Branch**:
   Call \`invoke_subagent\` with \`TypeName: "self"\`, \`Workspace: "branch"\`, \`Role: "Phase N: <Phase Name>"\`.
2. **OpenRouter MCP Delegation**:
   Instruct subagent to offload mechanical drafting (i18n, mock data, unit test boilerplate) to OpenRouter MCP (\`openai/gpt-oss-120b\`).
3. **Verify Gate**:
   Ensure automated tests/lint pass inside the branch.
4. **Anti-Looping Circuit Breaker**:
   If a phase fails verification twice consecutively, STOP immediately and report failure.
5. **Auto-Handoff**:
   Launch next phase subagent inside task branch with previous phase context. Repeat until all phases complete.

### 3. User Testing Gate (Mandatory)
- Output unified walkthrough: executed phases, modified files, diff summary, and manual test instructions.
- **STOP and ask**: *"All phases completed and automated tests passed in isolated branch. Please test the feature. Ready to merge and commit?"*

### 4. Merge, Auto-Commit & Cleanup
Upon user confirmation:
1. Switch to the base working branch: \`git checkout <target-branch>\`
2. Squash merge branch changes: \`git merge --squash <task-branch>\`
3. Auto-commit using Conventional Commits: \`git commit -m "<type>(<scope>): <concise description>"\`
4. Delete temporary task branch: \`git branch -D <task-branch>\`
5. Report final commit hash and status to user.
EOF

cat << 'EOF' > "$HOME/.gemini/config/skills/phase-plan/SKILL.md"
---
name: phase-plan
description: >-
  Interactive multi-phase planner and subagent orchestrator with branch isolation. Plans phases, requests approval, runs subagents in an isolated branch (Workspace: 'branch'), prompts user to test, squash-merges with Conventional Commit upon confirmation, and deletes temporary branch.
---

# Phase Plan (Interactive Branch-Isolated Orchestrator)

Plan complex tasks interactively, execute approved phases in an isolated branch (\`Workspace: 'branch'\`), prompt user for manual testing, squash-merge with Conventional Commit, and auto-delete scratch branches.
EOF
...
echo "----------------------------------------------------"
echo "Linux agent token-saving setup completed successfully!"
echo "Target Project: $PROJECT_DIR"
echo "Ensure 'OPENROUTER_API_KEY' is exported in ~/.bashrc or ~/.zshrc if using OpenRouter delegation."
echo "----------------------------------------------------"
