...
export default function TaxReportsModal({ isOpen, onClose }) {
  ...
  const handlePrintA4 = () => {
    if (activeTab === 'dpfo') {
      window.open(`/api/v1/reports/tax-statement/html?year=${selectedYear}`, '_blank');
    } else {
      window.open(`/api/v1/reports/vat-overview/html?year=${selectedYear}&period=${vatPeriod}`, '_blank');
    }
  };
  ...
  const handlePrintThermal = async () => {
    ...
    try {
      const dataToSend = activeTab === 'dpfo' ? taxData : vatData;
      const res = await fetch('/api/v1/printer/print-tax-report', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          reportType: activeTab,
          reportData: dataToSend || {},
          storeConfig: {}
        })
      });
      if (!res.ok) {
        const err = await res.json().catch(() => ({}));
        throw new Error(err.detail || 'Nepodařilo se vytisknout sestavu na pokladně.');
      }
      setThermalStatus({ type: 'success', text: 'Vytištěno na pokladně!' });
      setTimeout(() => setThermalStatus(null), 3500);
    } catch (e) {
      setThermalStatus({ type: 'error', text: e.message });
      setTimeout(() => setThermalStatus(null), 4500);
    } finally {
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
            ...
            <button
              ...
              title="Otevřít oficiální A4 daňový tiskopis pro tisk nebo uložení do PDF"
            >
              <FileText size={15} />
              <span>Tisknout A4 Výkaz</span>
            </button>

            <button
              type="button"
              onClick={handlePrintThermal}
              disabled={isPrintingThermal || isLoading}
              style={{
                display: 'inline-flex',
                alignItems: 'center',
                gap: '0.35rem',
                ...
                fontWeight: 700,
                fontSize: '0.82rem',
                cursor: isPrintingThermal ? 'wait' : 'pointer'
              }}
              title="Vytisknout daňový přehled na pokladní termotiskárnu (80mm/58mm)"
            >
              <Printer size={15} />
              <span>{isPrintingThermal ? 'Tisknu...' : 'Tisk na pokladnu'}</span>
            </button>
        ...
        {/* Thermal Print Status Feedback */}
        {thermalStatus && (
          <div style={{
            padding: '0.45rem 1.25rem',
            background: thermalStatus.type === 'success' ? 'rgba(16, 185, 129, 0.15)' : 'rgba(239, 68, 68, 0.15)',
            color: thermalStatus.type === 'success' ? '#10b981' : '#ef4444',
            fontSize: '0.82rem',
            fontWeight: 700,
            textAlign: 'center',
            borderBottom: '1px solid var(--border-color)'
