...
- `ManualKeypad.jsx`: Touch numeric pad coordinator with `touch-action: manipulation` zero-delay keys. Partitioned into two separate floating card boxes (`.pos-col-left` transparent flex column): top Keypad box (`.keypad-card-box` with 46px keys, standalone 36px item name input, inline borderless backspace on price readout, and `.keypad-bottom-dock` placeholder) and bottom-docked Auxiliary box (`.keypad-auxiliary-box` anchored with `margin-top: auto`). Decomposed into `/src/components/keypad/`:
