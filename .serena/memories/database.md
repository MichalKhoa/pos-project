...
- `SaleItemModel`: Individual line items linked to `SaleModel` foreign key (`name`, `quantity: float = 1.0`, `unit_price`, `vat_rate`, `discount_percent`). Preserved immutably as static snapshots; not foreign-key constrained to presets. Supports decimal quantities (kg) with 3-decimal precision.
